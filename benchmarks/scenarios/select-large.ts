...
export async function runSelectLarge(tcpClient: any, httpClient: any): Promise<BenchmarkResult> {
  console.log('  Running: SELECT 100,000 rows (5 iterations)...')

  const query = 'SELECT toUInt32(number) as n, toString(number) as str FROM system.numbers LIMIT 100000'
